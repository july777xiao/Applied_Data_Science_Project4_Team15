...
输入: ~/Downloads/raw/nyc_restaurant_panel.csv
输出: 
  ~/Downloads/raw/restaurant_clean.csv        全量清洗数据（用于failed预测）
  ~/Downloads/raw/restaurant_yelp_subset.csv  Yelp子集（用于high_rating预测）
"""
...
# ─────────────────────────────────────────────────────────────
# 3. score 缺失处理
#    更稳的判断：score为空 且 grade也为空 且 violation_count==0
#    → 这类是无实质检查结果的记录，直接删除
...
#    不删，不填，建模不用，可视化时按需过滤
# ─────────────────────────────────────────────────────────────
print("\n5. lat/lon: keeping as-is (not used in modeling)")
...
# ─────────────────────────────────────────────────────────────
# 7. Census 缺失
#    borough='0'已删，剩余Census缺失用borough均值填充
# ─────────────────────────────────────────────────────────────
...
#    - prev_score → 填 -1（"首次检查"标记）
#    - score_trend → 填 0（无变化）
#    - 新增 is_first_inspection 和 has_history 标记列
# ─────────────────────────────────────────────────────────────
...
if len(remaining) > 0:
    ...
    print("   None (except intentional lat/lon and yelp fields)")
...
print("\nCLEANING COMPLETE")
print("=" * 60)
print("Files for downstream use:")
